---
title: "Homework 3 - Arrange Tables"
...
knitr::opts_chunk$set(fig.width = 18, fig.height = 5) 
```
...
```{r Preparing the data to create the required charts}
df <- read.csv("nest-data.csv")
df1 <- read.csv("nest-weather.csv")
merged_data <- merge(df,df1)
mydata <- merged_data
mydata <- mydata %>% 
arrange(parse_date_time(Date, "mdy"))
view(mydata)
guestcount <- aggregate(mydata$Total.Guests~mydata$Church, FUN = sum)
view(guestcount)
churchguestcount <- t(guestcount[-1])
colnames(churchguestcount) <- guestcount[, 1]
view(churchguestcount)

```

```{r Q1 Chart,  fig.height=10}
...
```{r Q2 chart, fig.height = 10, fig.width=26}
